...
bool check_control_signals(Vcontrol_unit* control, int state) {
  ...
  uint16_t expected_mux_sel =
      (control->instruction >> 13) & 0x7;  // Bits [15:13]
  ...
  int en_0 = control->en_0, en_1 = control->en_1, en_2 = control->en_2,
      en_3 = control->en_3;
  int en_4 = control->en_4, en_5 = control->en_5, en_6 = control->en_6,
      en_7 = control->en_7;
  ...
  switch (state) {
    ...
    case 3:  // State1: expect en_s = 1 and mux_sel = instruction[15:13]
      ...
      is_correct &= (en_4 == 0 && en_5 == 0 && en_6 == 0 && en_7 == 0);
      break;
    case 4:  // State2: expect en_c = 1, mux_sel = instruction[12:10], alu_sel =
             // instruction[4:2]
    ...
    case 5:  // State3: expect correct enable signals based on instruction[15:13]
      ...
      switch (expected_mux_sel) {
        ...
        case 7:
          ...
          break;
        default:
          is_correct = false;  // Invalid mux selection
      }
      break;
    default:
...
int main(int argc, char** argv) {
  ...
  for (int instr = 0; instr < (1 << 16); ++instr) {
    ...
    control->run = 1;
    // Simulate the control unit for 4 clock cycles (enough to move through the
    // FSM)
    for (int cycle = 0; cycle < 6; ++cycle) {
      ...
      if (!correct) {
        cout << "Error: Incorrect control signals for instruction 0x" << hex
             << instr;
        cout << " at cycle " << dec << cycle << endl;
        exit(1);
      }
      if (control->done) {
        cout << "Done signal activated on instruction 0x" << hex << instr
             << endl;
      }
      do_clock_cycle();
    }
